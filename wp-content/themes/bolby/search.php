...
<h1 class="mt-0 mb-5 archive-header"><?php echo esc_attr('Search result for: ', 'bolby'); echo get_search_query(); ?></h1>
...
<div class="row">
	...
	<div class="col-md-<?php if( get_theme_mod('blog_sidebar', true) == true ) {echo '8';} else {echo '12';} ?>">
		...
		<div class="no-search-result shadow-dark padding-30 bg-white rounded">
			...
			<h3 class="mt-0 mb-4"><?php esc_attr_e('Nothing found', 'bolby'); ?></h3>

			<p class="clearfix mb-4"><?php esc_attr_e( 'Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'bolby' ); ?></p>

			<form class="form-inline search-form-default" role="search" method="get" id="searchform" action="<?php echo esc_url( home_url( '/' ) ); ?>">
				<input class="search-page form-control" type="text" aria-label="Search" name="s" placeholder="<?php esc_attr_e('Search...', 'bolby'); ?>" value="<?php echo get_search_query(); ?>" required>
				<button type="submit" class="btn btn-default ml-1"><?php esc_attr_e('Search', 'bolby'); ?></button>
			</form>
