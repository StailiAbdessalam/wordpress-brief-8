 ...
 */
class FilterHandler extends \AIOSEO\Vendor\Monolog\Handler\AbstractHandler
{
    /**
     ...
     */
    public function __construct($handler, $minLevelOrList = \AIOSEO\Vendor\Monolog\Logger::DEBUG, $maxLevel = \AIOSEO\Vendor\Monolog\Logger::EMERGENCY, $bubble = \true)
    {
     ...
     */
    public function setAcceptedLevels($minLevelOrList = \AIOSEO\Vendor\Monolog\Logger::DEBUG, $maxLevel = \AIOSEO\Vendor\Monolog\Logger::EMERGENCY)
    {
        if (\is_array($minLevelOrList)) {
            $acceptedLevels = \array_map('AIOSEO\\Vendor\\Monolog\\Logger::toMonologLevel', $minLevelOrList);
        } else {
            $minLevelOrList = \AIOSEO\Vendor\Monolog\Logger::toMonologLevel($minLevelOrList);
            $maxLevel = \AIOSEO\Vendor\Monolog\Logger::toMonologLevel($maxLevel);
            $acceptedLevels = \array_values(\array_filter(\AIOSEO\Vendor\Monolog\Logger::getLevels(), function ($level) use($minLevelOrList, $maxLevel) {
